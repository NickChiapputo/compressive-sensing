...
class AMP_net_Deblock(Module):
    def __init__(self,layer_num, A, is_cuda):
        super().__init__()
    ...
    def forward(self, inputs, output_layers):
        ...
        for n in range(output_layers):
            ...
            if self.is_cuda:
                ...
                X = z - torch.matmul(
                    (step * torch.matmul(torch.transpose(self.A,0,1), self.A)) - torch.eye(33 * 33).float(), noise)
...
def train(model, opt, train_loader, epoch, batch_size, CS_ratio,PhaseNum, is_cuda):
    model.train()
    n = 0
...
if __name__ == "__main__":
    ...
    for CS_ratio in CS_ratios:
        for PhaseNumber in PhaseNumbers:
            ...
            model = AMP_net_Deblock(PhaseNumber,A, is_cuda)
            ...
            for epoch in range(1, EpochNum + 1):
                train(model, opt, train_loader, epoch, batch_size, CS_ratio,PhaseNumber, is_cuda)
